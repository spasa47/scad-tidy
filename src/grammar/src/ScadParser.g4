...
operand : unaryOperator ? ID | integer | unaryOperator ? functionCall | LPAREN operation RPAREN;
negativeInt : MINUS NUMBER | PLUS negativeInt | MINUS positiveInt;
positiveInt : NUMBER | PI | PLUS positiveInt | MINUS negativeInt;

integer : positiveInt | negativeInt;

literalExpression : literal | operation | functionCall;

literal : integer | unaryOperator * ID | STRINGLITERAL | BOOL | vector;

assignmentOperator : EQ;


binaryOperator : PLUS | MINUS | TIMES | DIVISION | PERCENTSIGN;
unaryOperator : PLUS | MINUS;

function_declaration : FUNCTION ID LPAREN fun_params RPAREN EQ fun_body SEMI;

fun_params : | ( fun_param COMMA ) *fun_param;

fun_param : ID | ID EQ literalExpression;

fun_body : operation;

functionCall : ID LPAREN call_arguments RPAREN;

statement : debugableStatement | simpleStatement;

debugableStatement : ( HASH | PERCENTSIGN | TIMES ) simpleStatement;

simpleStatement : function_declaration | module_call | moduleDeclaration | variableAssignment
                  | ifElseStatement
                  | forStatement;

vector : LSQUARE literalExpression COMMA literalExpression COMMA literalExpression RSQUARE;

call_argument : literalExpression | ID EQ literalExpression;
...
ternaryOperator : predicates QUESTIONMARK( operation | ternaryOperator )
                      COLON( operation | ternaryOperator );

boolComparisonOperator : EQUALS | NOTEQUALS | GREATERTHAN | LESSTHAN | GREATERTHANEQUALS
                         | LESSTHANEQUALS;
boolBinaryOperator : LOGICAND | LOGICOR;

boolUnaryOperator : LOGICNOT;

predicate : BOOL | literalExpression boolComparisonOperator literalExpression
